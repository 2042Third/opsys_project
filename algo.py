    ...
    def upcpu(self):
        ...
        if self.cpu[1] == 0 and self.cpu[0] == 1:
            ...
            if self.time < self.rdn:
                print('time {}ms: Process {} (tau {}ms) completed a CPU burst; {} bursts to go '.format(self.gt(),self.cpu[2].getna(),self.cpu[2].gettau(), self.cpu[2].remburs()),end='')
                self.rdqprint()
...
def SRT2(data, alpha,lmda,switcht, processlist):
